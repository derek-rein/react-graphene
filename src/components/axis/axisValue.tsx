...
import { convertComponentsSpaceToChartSpace, EMPTY_VECTOR, getXY, zoomLevel } from '../../utils'
...
export function AxisValue() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      let bounce = Math.pow(10, zl - 2)
      let bounce_major = Math.pow(10, zl - 1)
      ...
      Array(Math.max(1, divisions_major)).fill(undefined).map((_, i) => {
        let value = oy + (bounce_major * i)
        let point = new DOMPoint(0, value).matrixTransform(state.matrix_y)
  ...
  return (
    <>
      <canvas className='axis axis-value rg-chart-axis-value' 
      ref={ref}
