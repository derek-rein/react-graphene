...
import {
  epochToString, YEAR_MONTH_ZOOM_LEVEL, ZoomLevel, zoomLevels,
} from './functions';
...
export function AxisTime() {
  ...
  const {
    state, dispatch, translateView, scaleView, variables, mouse,
  } = useChartContext();
  ...
  const draw = () => {
    ...
    ctx.strokeText(`${epochToString(state?.bounds?.width)} ${epochToString(state?.bounds?.width)}`, canvas.getBoundingClientRect().width - 200, 25);
  };
  ...
  const setZoomLevelForDensity = (density: number) => {
    ...
    for (const [maximalSpacing, zoomLevel] of zoomLevels.entries()) {
      ...
      if (maximalSpacing / size < density) { break; }
