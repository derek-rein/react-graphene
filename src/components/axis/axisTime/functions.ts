...
function makeMStepper(stepSize: number): Stepper {
    function stepper(val: number, n: number, first: boolean) {
        ...
        let d = utcfromtimestamp(val)
        let base0m = d.getUTCMonth() + n * stepSize - 1
        let d2 = new Date(d.getUTCFullYear() + Math.floor(base0m / 12), base0m % 12 + 1, 1)
...
function makeYStepper(stepSize: number): Stepper {
    function stepper(val: number, n: number, first: boolean) {
        if (val < MIN_REGULAR_TIMESTAMP || val > MAX_REGULAR_TIMESTAMP)
            return Infinity
        let d = utcfromtimestamp(val)
        let next_year = Math.floor(d.getUTCFullYear() / (n * stepSize) + 1) * (n * stepSize)
        if (next_year > 9999)
            return Infinity
        let next_date = new Date(next_year, 1, 1)
  ...
  class TickSpec {
    ...
    skipFactor(minSpc: number) {
      ...
      var factors = this.autoSkip
  
      while (true) {
        // not sure if this is translated
        for (var f: number, _pj_c = 0, _pj_a = factors, _pj_b = _pj_a.length; _pj_c < _pj_b; _pj_c += 1) {
          f = _pj_a[_pj_c];
          spc = this.spacing * f;
  ...
  export class ZoomLevel {
    ...
    // # current zoom level. This is used for auto skipping ticks.
      ...
      let valueSpecs: [number, number[]][] = []
      let allTicks: number[] = [];
    //   # back-project (minVal maxVal) to UTC, compute ticks then offset to
    //   # back to local time again

      let utcMin = minVal - this.utcOffset;
      let utcMax = maxVal - this.utcOffset;

      for (var spec: TickSpec, _pj_c = 0, _pj_a = this.tickSpecs, _pj_b = _pj_a.length; _pj_c < _pj_b; _pj_c += 1) {
        spec = _pj_a[_pj_c]
...
export const HOUR_MINUTE_ZOOM_LEVEL = new ZoomLevel([
    new TickSpec(DAY_SPACING, makeSStepper(DAY_SPACING), '%a %d'),
    new TickSpec(MINUTE_SPACING, makeSStepper(MINUTE_SPACING), '%H:%M',
             [1, 5, 15])
], "MMM 00")

export const HMS_ZOOM_LEVEL = new ZoomLevel([
    new TickSpec(SECOND_SPACING, makeSStepper(SECOND_SPACING), '%H:%M:%S',
             [1, 5, 15, 30])
], "99:99:99")

export const MS_ZOOM_LEVEL = new ZoomLevel([
    new TickSpec(MINUTE_SPACING, makeSStepper(MINUTE_SPACING), '%H:%M:%S'),
    new TickSpec(MS_SPACING, makeMSStepper(MS_SPACING), '%S.%f',
             [1, 5, 10, 25])
], "99:99:99")
