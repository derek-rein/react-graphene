...
import useChartContext, { usePlotable } from "../hooks";
...
const fromDecimal = (value: number) => {
  return value / DECIMAL
}
...
const Background: React.FC<IChartBackground> = ({color = '#222222'}) => {
  ...
  const {state, dispatch, mouse, matrix } = useChartContext()
  ...
  const drawVStats = (ctx: CanvasRenderingContext2D) => {
          ...
          const zl_blend = (height - y) / Math.pow(10, zl)
          const zl_blend_major = (height - y) / Math.pow(10, zl)
    
    
          let outerDist = Math.abs(oy - oh)
          let v_space = outerDist / 10
    
          let bounce = Math.pow(10, zl)/100
          let bounce_major = Math.pow(10, zl) / 10
    
          let divisions = Math.abs(oy - oh) / bounce
          let divisions_major = Math.abs(oy - oh) / bounce_major
  ...
  const drawHStats = (ctx: CanvasRenderingContext2D) => {
    ...
    const zl_blend = (width - x) / Math.pow(10, zl)
    const zl_blend_major = (width - x) / Math.pow(10, zl)


    let outerDist = Math.abs(ox - ow)
    let v_space = outerDist / 10

    let bounce = Math.pow(10, zl)/100
    let bounce_major = Math.pow(10, zl) / 10

    let divisions = Math.abs(ox - ow) / bounce
    let divisions_major = Math.abs(ox - ow) / bounce_major

    ctx.strokeStyle = `#ff0000`

    let duration = intervalToDuration({ start: 0, end: (width-x) * 1000 })
