import React, { createContext, Dispatch, useContext, useEffect, useLayoutEffect, useMemo, useReducer, useRef } from 'react';
import { usePlotable } from '.';
...
export const ChartContext: React.FC<IChartContext> = ({children, scrollSpeed = 1}) => {
  ...
  const scaleView = (vector: Vector, origin: MousePosition) => {
    if (containerRef.current === null) return
    mouse.bounds = containerRef.current.getBoundingClientRect();
    let offsetMatrix = new DOMMatrix().translateSelf(origin.x, origin.y)
